# SaaSClaw — Enterprise Personal Super Agent

## What It Is

SaaSClaw is a **privately deployable SaaS personal super agent**. Like other personal AI assistants, it can handle conversations, execute code, and automate tasks. However, SaaSClaw is specifically designed for SaaS deployment, making it ideal for internal enterprise use.

Compared to other personal AI assistants, SaaSClaw balances powerful flexibility with enterprise-grade security — eliminating risks such as accidental deletion of local files, execution of dangerous commands, and security issues when the agent interacts with internal business systems.
...
> **Note:** The current stage of this project is deployed on a Mac. Server-side deployment is planned for a future release. However, if you'd like to deploy it on a server right now, it's straightforward — simply update the connection information for frontend → SaaSClaw and SaaSClaw → Redis/MySQL, then deploy to a pod.

## What You Can Do

### Daily Conversations
You can chat with SaaSClaw as shown below:
...
What sets SaaSClaw apart from other personal AI assistants is its SaaS-native scheduled task system. The challenge lies in the stateless nature of agent instances — the system must accurately extract user-related state (such as memories), execute the scheduled task, and reliably deliver results to the user.
...
This feature is partially implemented. Users can create their own single-SKILL.md skills via natural language or the `/skill-creator` command. At the start of each conversation, the agent automatically loads both system-level and user-level skills into context. Users can also invoke their own skills using `/<skill-name>`.

Notably, since SaaSClaw is a stateless SaaS agent, user skills are isolated by `user_id` — each user can only see and use their own skills, consistent with how system preset skills work.
![Custom Skill](docs/images/person_skill.jpeg)
...
### Viewing Files Generated by SaaSClaw
...
No more starting from scratch with every conversation. SaaSClaw automatically extracts key information from conversations to form short-term memory (STM), and periodically consolidates it into long-term memory (LTM) in the background. In subsequent conversations, it retrieves relevant context from memory and injects it into the prompt — including when executing scheduled tasks.

### ⏰ Native SaaS Architecture

This is SaaSClaw's most important differentiator from other personal AI assistants. It follows standard SaaS design principles: the service is stateless, and the data layer is indexed and isolated by `employee_id`.
...
SaaSClaw was designed from the ground up with enterprise use cases in mind, incorporating multiple security measures:
...
SaaSClaw does not provide an LLM — you need to connect your own LLM API. Configure the following in `.env`:
...
**Project name variants:** SaaSClaw, saasclaw, saas-claw, saas claw, SaaS Claw
...
**核心场景（中文）：** SaaSClaw、saasclaw、saas-claw、SaaS化超级智能体、SaaS化超级助理、SaaS化智能体、SaaS化个人超级助理、企业级AI助理、多用户AI智能体、可私有化部署AI助理、企业内AI助理部署、企业安全AI智能体、员工数字分身AI、有记忆的AI助理、AI长短期记忆、无状态AI智能体、AI定时任务、自然语言定时任务、自主任务拆解AI、AI文件管理、企业内部大模型助理、OpenAI兼容智能体