...
''',
    ...
    # Upload generated PPTX files to MinIO when running inside the agent framework.
    # SA_EMPLOYEE_ID is injected by code_execute; skip upload silently in local dev.
    if success:
        _upload_artifacts(
            files=[p for p in [native_path, legacy_path] if p is not None and p.exists()],
            verbose=verbose,
        )
...
def _upload_artifacts(files: list[Path], verbose: bool) -> None:
    """Upload generated PPTX files to MinIO via upload_artifact.py."""
    import json
    import subprocess

    employee_id = os.environ.get("SA_EMPLOYEE_ID", "")
    if not employee_id:
        return  # not running inside agent framework — skip upload

    upload_script = str(Path(__file__).resolve().parents[1] / "upload_artifact.py")
    if not Path(upload_script).exists():
        if verbose:
            print("  [warn] upload_artifact.py not found; skipping MinIO upload")
        return
    ...
    for pptx_path in files:
        ...
        if result.returncode == 0:
            try:
                ...
                print(f"\n{data.get('message', f'Uploaded: {pptx_path.name}')}")
            except Exception:
                print(f"\n  Uploaded: {pptx_path.name}")
        else:
            if verbose:
                print(f"  [warn] MinIO upload failed for {pptx_path.name}: {result.stderr.strip()}")
