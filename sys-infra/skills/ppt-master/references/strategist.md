...
> **Auto-bypass rule**: When the task is trivially simple — the user request clearly fits on **≤2 pages** with no ambiguity about topic, audience, or format — skip the confirmation pause. Auto-decide all eight items yourself with sensible defaults (PPT 16:9, General Versatile style, clean color scheme, no icons unless obviously needed, no external images unless the user supplied them), output `design_spec.md` + `spec_lock.md`, and proceed directly to SVG generation. For multi-page decks, ambiguous briefs, or anything that benefits from user input, stop and confirm.

> **Execution discipline**: This is the last BLOCKING checkpoint in the pipeline. After confirmation (or auto-bypass), complete the Design Spec and proceed to image generation / SVG / post-processing without further pauses.
